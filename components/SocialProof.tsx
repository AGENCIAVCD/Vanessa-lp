    ...
    <section className="px-5 py-14 sm:px-8 sm:py-16 lg:px-10" id="prova-social">
      <div className="mx-auto max-w-7xl">
        <SectionReveal className="max-w-3xl">
          ...
          <p className="mt-5 font-body text-base leading-8 text-ink/76" data-reveal-item>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(320px,0.85fr)_minmax(0,1.15fr)]">
          <SectionReveal className="h-full">
            <div
              className="relative h-full overflow-hidden rounded-[2.5rem] border border-rosegold/20 bg-white shadow-soft"
              data-reveal-item
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white">
                <p className="font-body text-[10px] uppercase tracking-[0.34em] text-white/70">
          ...
          <SectionReveal className="grid gap-4 sm:grid-cols-2">
            {transformationCards.map((card) => (
              <article
                className="flex min-h-[220px] flex-col justify-between rounded-[2rem] border border-dashed border-rosegold/40 bg-white/72 p-5 shadow-soft backdrop-blur"
                data-reveal-item
                key={card}
              >
                <div>
                  <p className="font-body text-[10px] uppercase tracking-[0.28em] text-forest/58">
                    Placeholder premium
                  </p>
                  <h3 className="mt-3 font-display text-2xl leading-tight text-forest">{card}</h3>
                </div>
                <p className="font-body text-sm leading-6 text-ink/62">
            ...
            {testimonials.map((testimonial) => (
              <article
                className="rounded-[2rem] border border-rosegold/18 bg-forest p-5 text-white shadow-soft"
                data-reveal-item
              ...
              >
                ...
                <p className="mt-4 font-body text-sm leading-7 text-white/80">“{testimonial.text}”</p>
                <p className="mt-5 font-body text-xs font-semibold uppercase tracking-[0.18em] text-white/70">
                  {testimonial.name}
                </p>
              </article>
            ))}
